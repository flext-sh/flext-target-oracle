...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('flext_target_oracle.log')
    ]
)
...
class ProductionConfig:
    ...
    def create_from_environment() -> FlextOracleTargetConfig:
        ...
        logger.info(f"Production configuration created: {oracle_host}:{oracle_port}/{oracle_service}")
        logger.info(f"Target schema: {default_target_schema}, Batch size: {batch_size}")
        logger.info(f"Load method: {load_method}, Connection timeout: {connection_timeout}s")
...
class ProductionTargetManager:
    ...
    def __init__(self, config: FlextOracleTargetConfig):
        ...
        """
        self.config = config
        self.target: Optional[FlextOracleTarget] = None
    ...
    async def process_singer_stream(self, messages: list[Dict[str, Any]]) -> FlextResult[Dict[str, Any]]:
        ...
        logger.info(f"Processing Singer stream with {len(messages)} messages")
        ...
        try:
            for i, message in enumerate(messages):
                ...
                logger.debug(f"Processing message {i+1}/{len(messages)}: {message_type}")
                ...
                if result.is_success:
                    ...
                    logger.error(f"Message {i+1} processing failed: {result.error}")
            ...
            logger.info(f"Stream processing completed in {processing_duration:.2f} seconds")
    ...
    async def health_check(self) -> FlextResult[Dict[str, Any]]:
        ...
        try:
            ...
            health_status["metrics"] = self.target._get_implementation_metrics() if self.target else {}
...
async def demonstrate_production_setup() -> None:
    ...
    try:
        ...
        if init_result.is_failure:
            logger.error(f"Production initialization failed: {init_result.error}")
            return
        ...
        if health_result.is_success:
            ...
            logger.info(f"Oracle connectivity: {health_data['checks'].get('oracle_connectivity', 'unknown')}")
        else:
        ...
        if processing_result.is_success:
            ...
            logger.info(f"Processing duration: {stats.get('processing_duration_seconds', 0):.2f}s")
            ...
            logger.error(f"Production processing failed: {processing_result.error}")
        
        # Step 6: Final health check
        logger.info("Step 6: Performing final health check")
        final_health = await manager.health_check()
        if final_health.is_success:
            logger.info(f"Final health status: {final_health.data['status']}")
        ...
        if shutdown_result.is_success:
            ...
            logger.error(f"Shutdown issues: {shutdown_result.error}")
            
    except Exception as e:
        logger.exception(f"Production demonstration failed: {e}")
        raise
    ...
    for i in range(1, 101):  # 100 sample orders
        record_message = {
            ...
            "record": {
                ...
                "order_date": (base_date + datetime.timedelta(hours=i)).isoformat() + "Z",
                "product_sku": f"SKU-{(i % 10) + 1:03d}",
                "quantity": (i % 5) + 1,
                "unit_price": round(19.99 + (i % 100), 2),
                "total_amount": round((19.99 + (i % 100)) * ((i % 5) + 1), 2),
                "order_status": ["pending", "processing", "shipped", "delivered"][i % 4],
                "shipping_address": f"{i} Main St, City {i % 10}, State",
                "created_at": (base_date + datetime.timedelta(hours=i)).isoformat() + "Z",
                "updated_at": (base_date + datetime.timedelta(hours=i, minutes=30)).isoformat() + "Z"
            }
        }
    ...
    state_message = {
        ...
        "value": {
            "bookmarks": {
                "customer_orders": {
                    "last_order_id": 100,
                    "last_updated": (base_date + datetime.timedelta(hours=100)).isoformat() + "Z"
                }
            }
        }
    }
...
def main() -> None:
    ...
    try:
        ...
        logger.exception(f"Production setup example failed: {e}")
        sys.exit(1)
