...
import pytest
from flext_target_oracle import FlextTargetOracle, FlextTargetOracleLoader, FlextTargetOracleSettings, LoadMethod, m, t
from sqlalchemy import text
from sqlalchemy.engine import Engine

def _schema_parts(message: Mapping[str, t.ContainerValue]) -> tuple[Mapping[str, t.JsonValue], list[str] | None]:
...
class TestOracleIntegration:
    ...
    def test_create_simple_table(self, connected_loader: FlextTargetOracleLoader, oracle_engine: Engine, simple_schema: dict[str, t.ContainerValue]) -> None:
        ...
        table_res = connected_loader.ensure_table_exists(stream_name, schema_dict, key_props)
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_insert_and_retrieve_data(self, connected_loader: FlextTargetOracleLoader, oracle_engine: Engine, simple_schema: dict[str, t.ContainerValue]) -> None:
        """Test inserting data and retrieving it."""
        stream_name = 'test_insert'
        schema_dict, key_props = _schema_parts(simple_schema)
        create_res = connected_loader.ensure_table_exists(stream_name, schema_dict, key_props)
        assert create_res.is_success
        records: list[Mapping[str, t.JsonValue]] = [{'id': 1, 'name': 'John Doe', 'email': 'john@example.com'}, {'id': 2, 'name': 'Jane Smith', 'email': 'jane@example.com'}]
        result = connected_loader.insert_records(stream_name, records)
        assert result.is_success
        with oracle_engine.connect() as conn:
            cursor = conn.execute(text('SELECT id, name, email FROM test_insert ORDER BY id'))
            rows = list(cursor)
            assert len(rows) == 2
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_merge_mode_updates(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine, simple_schema: dict[str, t.ContainerValue]) -> None:
        ...
        assert table_res.is_success
        initial_records: list[Mapping[str, t.JsonValue]] = [{'id': 1, 'name': 'Original Name', 'email': 'original@example.com'}]
        insert_result = loader.insert_records(stream_name, initial_records)
        assert insert_result.is_success
        updated_records: list[Mapping[str, t.JsonValue]] = [{'id': 1, 'name': 'Updated Name', 'email': 'updated@example.com'}]
        result = loader.insert_records(stream_name, updated_records)
        assert result.is_success
        with oracle_engine.connect() as conn:
            cursor_result = conn.execute(text('SELECT name, email FROM test_merge WHERE id = 1'))
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_bulk_insert_performance(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine) -> None:
        """Test bulk insert with large dataset."""
        oracle_config = oracle_config.model_copy(update={'load_method': LoadMethod.BULK_INSERT, 'batch_size': 1000})
        loader = FlextTargetOracleLoader(oracle_config)
        assert loader.connect().is_success
        stream_name = 'test_bulk'
        schema: dict[str, t.JsonValue] = {'type': 'object', 'properties': {'id': {'type': 'integer'}, 'data': {'type': 'string'}, 'value': {'type': 'number'}}}
        key_properties_list = ['id']
        schema_dict: Mapping[str, t.JsonValue] = schema
        key_props: list[str] | None = key_properties_list
        table_res = loader.ensure_table_exists(stream_name, schema_dict, key_props)
        assert table_res.is_success
        records: list[Mapping[str, t.JsonValue]] = [{'id': i, 'data': f'Bulk test data {i}', 'value': i * 1.5} for i in range(5000)]
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_json_storage_mode(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine, nested_schema: dict[str, t.ContainerValue]) -> None:
        """Test JSON storage mode with nested data."""
        oracle_config = oracle_config.model_copy(update={})
        oracle_config = oracle_config.model_copy(update={'storage_mode': 'json', 'json_column_name': 'json_data'})
        loader = FlextTargetOracleLoader(oracle_config)
        assert loader.connect().is_success
        ...
        assert create_res.is_success
        record = {'id': 1, 'customer': {'id': 100, 'name': 'Acme Corp', 'address': {'street': '123 Main St', 'city': 'objecttown', 'zip': '12345'}}, 'items': [{'product_id': 1, 'quantity': 2, 'price': 99.99}, {'product_id': 2, 'quantity': 1, 'price': 149.99}], 'total': 349.97}
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_column_ordering(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine) -> None:
        """Test column ordering in created tables."""
        oracle_config = oracle_config.model_copy(update={'column_ordering': 'alphabetical', 'column_order_rules': {'primary_keys': 1, 'regular_columns': 2, 'audit_columns': 3, 'sdc_columns': 4}})
        loader = FlextTargetOracleLoader(oracle_config)
        assert loader.connect().is_success
        stream_name = 'test_ordering'
        schema: dict[str, t.JsonValue] = {'type': 'object', 'properties': {'id': {'type': 'integer'}, 'zebra_field': {'type': 'string'}, 'alpha_field': {'type': 'string'}, 'created_at': {'type': 'string', 'format': 'date-time'}, 'updated_at': {'type': 'string', 'format': 'date-time'}}}
        key_properties_list = ['id']
        schema_dict: Mapping[str, t.JsonValue] = schema
        key_props: list[str] | None = key_properties_list
        table_res = loader.ensure_table_exists(stream_name, schema_dict, key_props)
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_truncate_before_load(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine, simple_schema: dict[str, t.ContainerValue]) -> None:
        ...
        create_res = loader.ensure_table_exists(stream_name, schema_dict, key_props)
        assert create_res.is_success
        insert_initial = loader.insert_records(stream_name, [{'id': 1, 'name': 'Initial'}])
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_custom_indexes(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine, simple_schema: dict[str, t.ContainerValue]) -> None:
        """Test creation of custom indexes."""
        oracle_config = oracle_config.model_copy(update={'custom_indexes': {'test_indexes': [{'name': 'IDX_EMAIL_UNIQUE', 'columns': ['EMAIL'], 'unique': True}, {'columns': ['NAME', 'CREATED_AT']}]}})
        loader = FlextTargetOracleLoader(oracle_config)
        assert loader.connect().is_success
...
class TestOracleTargetE2E:
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_full_singer_workflow(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine, singer_messages: list[dict[str, t.ContainerValue]]) -> None:
    ...
    @pytest.mark.usefixtures('_clean_database')
    def test_column_mapping_and_filtering(self, oracle_config: FlextTargetOracleSettings, oracle_engine: Engine) -> None:
        """Test column mapping and filtering features."""
        oracle_config = oracle_config.model_copy(update={'column_mappings': {'users': {'name': 'full_name', 'email': 'email_address'}}, 'ignored_columns': ['password', 'internal_id']})
        target = FlextTargetOracle(config=oracle_config)
        target.initialize()
        schema_msg = {'type': 'SCHEMA', 'stream': 'users', 'schema': {'type': 'object', 'properties': {'id': {'type': 'integer'}, 'name': {'type': 'string'}, 'email': {'type': 'string'}, 'password': {'type': 'string'}, 'internal_id': {'type': 'string'}}}, 'key_properties': ['id']}
        target.execute(json.dumps(schema_msg))
        record_msg = {'type': 'RECORD', 'stream': 'users', 'record': {'id': 1, 'name': 'John Doe', 'email': 'john@example.com', 'password': 'secret123', 'internal_id': 'INT-001'}}
        ...
        with oracle_engine.connect() as conn:
            ...
            result = conn.execute(text('SELECT full_name, email_address FROM users WHERE id = 1'))
            row = result.fetchone()
            assert row is not None
