...
class TestSingerWorkflowE2E:
    ...
    def _target(self) -> FlextTargetOracle:
        config = FlextTargetOracleSettings(oracle_host='localhost', oracle_service_name='XE', oracle_user='test', oracle_password='test')
    ...
    def test_complete_singer_flow(self) -> None:
        target = self._target()
        schema = {'type': 'SCHEMA', 'stream': 'orders', 'schema': {'type': 'object', 'properties': {'id': {'type': 'integer'}, 'amount': {'type': 'number'}}}, 'key_properties': ['id']}
        record = {'type': 'RECORD', 'stream': 'orders', 'record': {'id': 1, 'amount': 10.5}}
        ...
        assert target.process_singer_message(m.TargetOracle.SingerStateMessage.model_validate(json.loads(json.dumps(state)))).is_success
