...
class TestOracleTarget:
    ...
    def test_discover_catalog_uses_registered_schemas(self, target: FlextTargetOracle) -> None:
        schema_message = m.TargetOracle.SingerSchemaMessage.model_validate({'type': 'SCHEMA', 'stream': 'users', 'schema': {'type': 'object', 'properties': {'id': {'type': 'integer'}}}, 'key_properties': ['id']})
    ...
    def test_process_record_and_state_messages(self, target: FlextTargetOracle) -> None:
        schema_message = m.TargetOracle.SingerSchemaMessage.model_validate({'type': 'SCHEMA', 'stream': 'users', 'schema': {'type': 'object', 'properties': {'id': {'type': 'integer'}}}})
        record_message = m.TargetOracle.SingerRecordMessage.model_validate({'type': 'RECORD', 'stream': 'users', 'record': {'id': 1}})
        state_message = m.TargetOracle.SingerStateMessage.model_validate({'type': 'STATE', 'value': {'bookmarks': {'users': 1}}})
        assert target.process_singer_message(schema_message).is_success
    ...
    def test_process_singer_messages_flushes_loader(self, target: FlextTargetOracle) -> None:
        messages: list[m.TargetOracle.SingerSchemaMessage | m.TargetOracle.SingerRecordMessage | m.TargetOracle.SingerStateMessage | m.TargetOracle.SingerActivateVersionMessage] = [m.TargetOracle.SingerSchemaMessage.model_validate({'type': 'SCHEMA', 'stream': 'users', 'schema': {'type': 'object', 'properties': {'id': {'type': 'integer'}}}}), m.TargetOracle.SingerRecordMessage.model_validate({'type': 'RECORD', 'stream': 'users', 'record': {'id': 1}}), m.TargetOracle.SingerStateMessage.model_validate({'type': 'STATE', 'value': {'offset': 1}})]
    ...
    def test_invalid_json_payload_maps_to_processing_failure(self, target: FlextTargetOracle) -> None:
        result = target.execute('{ invalid }')
        assert result.is_success
        parse_result = target.write_record('{"type": "RECORD", "stream": "users", "record": "bad"}')
    ...
    def test_write_record_inserts_oracle_record(self, target: FlextTargetOracle) -> None:
        connection = MagicMock()
        cursor = MagicMock()
        connection.cursor.return_value.__enter__.return_value = cursor
        with patch.object(target, '_connect_oracle', return_value=connection):
            result = target.write_record(json.dumps({'type': 'RECORD', 'stream': 'users', 'record': {'id': 1}}))
        assert result.is_success
