...
def loader_config() -> FlextTargetOracleSettings:
    ...
    return FlextTargetOracleSettings(oracle_host='localhost', oracle_port=1521, oracle_service_name='XE', oracle_user='test_user', oracle_password='test_password', default_target_schema='TEST_SCHEMA', batch_size=2, use_bulk_operations=True)

def test_loader_execute_returns_ready_payload(loader_config: FlextTargetOracleSettings) -> None:
    """execute() should return a typed readiness payload."""
    loader = FlextTargetOracleLoader(loader_config)
    result = loader.execute()
    assert isinstance(result.is_success, bool)

def test_load_record_buffers_and_finalize(loader_config: FlextTargetOracleSettings) -> None:
...
def test_ensure_table_exists_returns_result(loader_config: FlextTargetOracleSettings) -> None:
    """ensure_table_exists should always return FlextResult[bool]."""
    loader = FlextTargetOracleLoader(loader_config)
    schema: Mapping[str, t.JsonValue] = {'type': 'object', 'properties': {'id': {'type': 'integer'}}}
