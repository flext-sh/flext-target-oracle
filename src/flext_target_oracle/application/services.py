...
class SingerTargetService:
    ...
    async def _handle_schema(self, record: SingerRecord) -> ServiceResult[Any]:
        """Handle SCHEMA message."""
        if not record.record_schema or not record.stream:
            return ServiceResult.fail(
                "Schema message missing schema or stream"
            )
    ...
    async def _handle_record(self, record: SingerRecord) -> ServiceResult[Any]:
        """Handle RECORD message."""
        if not record.record or not record.stream:
            return ServiceResult.fail(
                "Record message missing data or stream"
            )
    ...
    async def _handle_state(self, record: SingerRecord) -> ServiceResult[Any]:
    ...
    async def finalize_all_streams(self) -> ServiceResult[Any]:
...
class OracleLoaderService:
    ...
    async def _flush_batch(self, stream_name: str) -> ServiceResult[Any]:
    ...
    async def _create_table(
        ...
        try:
            ...
            if not columns:
                return ServiceResult.fail(
                    f"Schema validation failed: No valid columns found in schema for table {table_name}. Schema must contain at least one valid property."
                )
    ...
    async def _insert_batch(
        ...
        try:
            ...
            if existing_columns is None:
                return ServiceResult.fail(
                    "No columns returned from table query"
                )
    ...
    async def _get_table_columns(self, table_name: str) -> ServiceResult[Any]:
    ...
    async def _insert_batch_structured(
        ...
        if params is None:
            return ServiceResult.fail(
                "No parameters returned from build process"
            )
