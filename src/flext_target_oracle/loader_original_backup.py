...
Key Components:
    FlextTargetOracleLoader: Main data loading service with batch processing
...
# Oracle constants - use ONLY from flext-db-oracle, no duplication
MAX_ORACLE_IDENTIFIER_LENGTH = FlextOracleDbConstants.MAX_IDENTIFIER_LENGTH
...
class OracleConnectionManager:
    ...
    def connect(self) -> FlextResult[None]:
        """Establish Oracle connection using flext-db-oracle API context manager."""
        try:
            # flext-db-oracle uses context manager pattern, not direct connect()
            # Connection is established when entering context, not explicitly
            self._connection = None
            self._metadata_manager = None

            logger.info(
                "Oracle connection manager ready: %s:%s/%s",
            ...
            msg = f"Failed to prepare Oracle connection: {e}"
            raise FlextTargetOracleConnectionError(
                msg,
                host=self.config.oracle_host,
                port=self.config.oracle_port,
...
class OracleSchemaManager:
    ...
    def map_json_type_to_oracle(
        ...
        Uses flext-db-oracle constants for consistent type mapping.
        """
        # Map JSON types to Oracle types using dictionary lookup
        type_mapping = {
            "integer": f"NUMBER({DEFAULT_NUMBER_PRECISION},0)",
            "number": f"NUMBER({DEFAULT_NUMBER_PRECISION},{DEFAULT_NUMBER_SCALE})",
            "boolean": f"NUMBER({BOOLEAN_PRECISION},{BOOLEAN_SCALE})",
        }

        # Handle string type with format-specific mapping
        if json_type == "string":
            format_mapping = {
                "date": ORACLE_DATE_TYPE,
                "date-time": ORACLE_TIMESTAMP_TYPE,
                "datetime": ORACLE_TIMESTAMP_TYPE,
            }
            return format_mapping.get(format_type, f"VARCHAR2({DEFAULT_STRING_LENGTH})")

        # Handle object and array types
        if json_type in {"object", "array"}:
            return FlextOracleDbConstants.DataTypes.CLOB_TYPE

        # Return mapped type or default to VARCHAR2 for unknown types
        return type_mapping.get(json_type, f"VARCHAR2({DEFAULT_STRING_LENGTH})")
...
class OracleSQLGenerator:
    ...
    def build_merge_statement(
        ...
        if not self._is_valid_identifier(table_name):
            ...
            raise FlextTargetOracleProcessingError(msg)
...
class FlextTargetOracleLoader:
    ...
    def __init__(self, config: FlextTargetOracleConfig) -> None:
        ...
        Args:
            config: Validated FlextTargetOracleConfig with Oracle connection
    ...
    def _create_table_from_schema(
        ...
        except FlextTargetOracleSchemaError:
            raise
        except Exception as e:
    ...
    def _map_json_type_to_oracle(
        ...
        Returns:
            Oracle type string (e.g., 'NUMBER(10,2)', 'VARCHAR2(100)')
        ...
        if (
            ...
            return self.config.column_type_overrides[stream_name][prop_name]
        ...
        if self.config.type_mappings and json_type in self.config.type_mappings:
            return self.config.type_mappings[json_type]

        # Default type mapping using flext-db-oracle constants
        if json_type == "integer":
            return f"NUMBER({DEFAULT_NUMBER_PRECISION},0)"
        if json_type == "number":
        ...
        if json_type == "string":
            ...
            if max_length > self.config.use_clob_threshold:
                return FlextOracleDbConstants.DataTypes.CLOB_TYPE
            return f"VARCHAR2({min(max_length, 32767)})"
        if json_type in {"object", "array"}:
            return FlextOracleDbConstants.DataTypes.CLOB_TYPE  # Store as JSON
    ...
    async def load_record(
        ...
        Example:
            ...
            >>> loader = FlextTargetOracleLoader(config_with_batch_size_100)
            >>>
    ...
    def _build_merge_statement(
        ...
        # Build MERGE statement using config object
        merge_config = MergeStatementConfig(
        ...
        merge_result = self.oracle_api.build_merge_statement(merge_config)
