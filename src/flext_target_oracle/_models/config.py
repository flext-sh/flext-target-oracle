...
class FlextTargetOracleConfigModels:
    ...
    class Defaults(BaseModel):
        ...
        commit_interval: int = Field(
            ge=1,
            description="Commit interval in rows.",
        )
        ...
        load_method: str = Field(
            description="Default Oracle load strategy.",
        )
        storage_mode: str = Field(
            description="Default record storage mode.",
        )
        json_column_name: str = Field(
            description="JSON payload column name.",
        )
        truncate_before_load: bool = Field(
            description="Whether to truncate before load.",
        )
        use_bulk_operations: bool = Field(
            description="Whether to use bulk operations.",
        )
    ...
    class CommandTypes(BaseModel):
        ...
        validate_command: str = Field(
            description="Validate command identifier.",
        )
        load_command: str = Field(
            description="Load command identifier.",
        )
        about_command: str = Field(
            description="About command identifier.",
        )
