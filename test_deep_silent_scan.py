...
def analyze_exception_handler(
    file_path: str, line_num: int, lines: list[str]
) -> dict[str, Any]:
    ...
    for i in range(line_num, min(line_num + 10, len(lines))):
        ...
        if line and not line.startswith("#"):
            ...
            if (
                any(
                    keyword in line
                    for keyword in ["def ", "class ", "try:", "except", "finally:"]
                )
                and i > line_num
            ):  # Não parar na própria linha except
                break
    ...
    if re.search(r"except.*:\s*return", handler_code) and not any(
        keyword in handler_code
        for keyword in ["print", "log", "error", "warning", "debug"]
    ):
    ...
    if re.search(r"except.*:\s*[a-zA-Z_]\w*\s*=", handler_code) and not any(
        keyword in handler_code for keyword in ["print", "log", "error", "warning"]
    ):
        analysis["suspicious"] = True
    ...
    if re.search(r"except.*:\s*\w+\s*\+=\s*1", handler_code) and not any(
        keyword in handler_code for keyword in ["print", "log", "error", "warning"]
    ):
        analysis["suspicious"] = True
    ...
    if re.search(r"except.*:\s*\w+\.append\(", handler_code) and not any(
        keyword in handler_code for keyword in ["print", "log", "error", "warning"]
    ):
        analysis["suspicious"] = True
    ...
    if any(
        word in handler_code for word in ["close()", "cleanup()", "dispose()"]
    ) and not any(
        keyword in handler_code for keyword in ["print", "log", "error", "warning"]
    ):
        analysis["suspicious"] = True
    ...
    if len(handler_lines) <= 2 and not any(
        keyword in handler_code
        for keyword in ["print", "log", "error", "warning", "raise", "pass"]
    ):
        analysis["suspicious"] = True
    ...
    if re.search(
        r"#.*ignore|#.*skip|#.*continue|#.*ok", handler_code, re.IGNORECASE
    ) and not any(
        keyword in handler_code for keyword in ["print", "log", "error", "warning"]
    ):
        analysis["suspicious"] = True
...
def main():
    ...
    file_patterns = ["flext_target_oracle/*.py", "tests/*.py"]
    ...
    for pattern in file_patterns:
        ...
        for file_path in files:
            ...
            if file_path.endswith(
                ("test_deep_silent_scan.py", "test_zero_silent_errors_FINAL.py")
            ):
                continue
    ...
    if all_issues:
        ...
        for severity in ["high", "medium", "low"]:
            ...
            if issues_of_severity:
                issue_count = len(issues_of_severity)
                print(f"\n🚨 SEVERIDADE {severity.upper()} ({issue_count} issues):")
