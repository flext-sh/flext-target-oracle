...
def test_real_data_insertion():
    ...
    try:
        ...
        sink = target.get_sink("test_url_filtering", schema=schema_with_url["schema"], key_properties=["id"])
        
        print(f'📊 Schema original: {len(schema_with_url["schema"]["properties"])} campos')
        print(f'📊 Campos originais: {list(schema_with_url["schema"]["properties"].keys())}')
        ...
        with connector._engine.connect() as conn:
            result = conn.execute(text('SELECT COUNT(*) FROM TEST_URL_FILTERING'))
            count = result.scalar()
            print(f'📊 Registros inseridos: {count}')
            
            if count > 0:
                result = conn.execute(text('SELECT * FROM TEST_URL_FILTERING WHERE ROWNUM <= 1'))
                for row in result:
